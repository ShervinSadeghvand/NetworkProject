...
l=0
Id=0
N={'Location':[], 'Degrees':0}
nodes={Id:{'Location':[], 'Degrees':None}}
...
class Graph:
    ...
    class Node:  
        def AddingNode(self):
            ...
            nodes[Id]['Degrees']=int(input("enter number of degree(s) = "))
            Id+=1 
    ...
    class Link:
        ...
        def DeletingLink(self,InNodeId,OutNodeId):
            ...
            self.OutNodeId=OutNodeId
            for i in range(len(links)):
                if links[i]['InNodeId']==InNodeId or links[i]['OutNodeId']==OutNodeId:
                    del links[i]
                    for j in range(len(links)-i):
                        links[j+i]=links.pop(j+1+i)
...
while MenuChoose!=0:
    ...
    if MenuChoose==2:
        ...
        if Node_Del_choose==1:  ## its not done yet
    ...
    if MenuChoose==4:
        ...
        for i in range(Num_Of_Link):
            LINK.AddingLink(int(input("enter InNodeID : ")),int(input("enter OutNodeID : ")))
    if MenuChoose==5:
        LinkDelMenu()
        Link_Del_choose=int(input("enter the number from above list : "))
        if Link_Del_choose==1:  
            num=int(input("enter the ID you want to delete its node : "))
            del links[num]
            for j in range(len(links)-i):
                links[j+i]=links.pop(j+1+i)
